<div class="p-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <!-- Attachments -->
        @if(count($notification->attachmentItems()) > 0)
            <div class="px-6 pb-6">
                <h4 class="text-sm font-semibold text-gray-900 mb-3 flex items-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path>
                    </svg>
                    Attachments ({{ count($notification->attachmentItems()) }})
                </h4>
                <div class="space-y-2">
                    @foreach($notification->attachmentItems() as $attachment)
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                            ...
                            @if($attachment['path'])
                                <a 
                                    href="{{ route('notifications.attachment.download', ['notification' => $notification->id, 'index' => $loop->index]) }}"
                                    download
                            ...
                            @else
                                <span class="text-sm text-gray-500">Download unavailable</span>
                            @endif
                        </div>
                    @endforeach
                </div>
