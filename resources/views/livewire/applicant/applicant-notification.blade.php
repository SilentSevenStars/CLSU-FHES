    ...
    @endif
        ...
        @if($selectedNotification)
            <div class="fixed inset-0 z-50 overflow-y-auto" x-data="{ show: true }" x-show="show" x-cloak>
                ...
                @if(count($selectedNotification->attachmentItems()) > 0)
                    <div class="mb-6">
                        <h4 class="text-sm font-semibold text-gray-900 mb-3">Attachments</h4>
                        <div class="space-y-2">
                            @foreach($selectedNotification->attachmentItems() as $attachment)
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                    ...
                                    @if($attachment['path'])
                                        <a 
                                            href="{{ route('notifications.attachment.download', ['notification' => $selectedNotification->id, 'index' => $loop->index]) }}"
                                            download
