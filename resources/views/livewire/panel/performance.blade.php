                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-gray-700">
                                    ...
                                    He is given 20 minutes to do the task. The following guide may serve as indicates
                                    in grading the applicant.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form wire:submit.prevent="confirmSubmission">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="flex justify-between items-center py-4 border-b">
                                <p class="text-gray-700 flex-1 pr-8">1.1 His/her knowledgeability and mastery of the subject matter</p>
                                <div class="flex gap-12">
                                    @foreach(['VS', 'S', 'F', 'P', 'NI'] as $value)
                                    <label class="flex items-center justify-center w-16">
                                        <input type="radio" wire:model="question1" value="{{ $value }}" class="w-6 h-6 cursor-pointer">
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            @error('question1') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                            <!-- Question 1.2 -->
                            <div class="flex justify-between items-center py-4 border-b">
                                <p class="text-gray-700 flex-1 pr-8">1.2 His/her way of communication has ideas to students</p>
                                <div class="flex gap-12">
                                    @foreach(['VS', 'S', 'F', 'P', 'NI'] as $value)
                                    <label class="flex items-center justify-center w-16">
                                        <input type="radio" wire:model="question2" value="{{ $value }}" class="w-6 h-6 cursor-pointer">
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            @error('question2') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                            <!-- Question 1.3 -->
                            <div class="flex justify-between items-center py-4 border-b">
                                <p class="text-gray-700 flex-1 pr-8">1.3 His/her skill in initiating the lesson</p>
                                <div class="flex gap-12">
                                    @foreach(['VS', 'S', 'F', 'P', 'NI'] as $value)
                                    <label class="flex items-center justify-center w-16">
                                        <input type="radio" wire:model="question3" value="{{ $value }}" class="w-6 h-6 cursor-pointer">
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            @error('question3') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                            <!-- Question 1.4 -->
                            <div class="flex justify-between items-center py-4 border-b">
                                <p class="text-gray-700 flex-1 pr-8">1.4 His/her way of motivating students to participate actively in classroom task</p>
                                <div class="flex gap-12">
                                    @foreach(['VS', 'S', 'F', 'P', 'NI'] as $value)
                                    <label class="flex items-center justify-center w-16">
                                        <input type="radio" wire:model="question4" value="{{ $value }}" class="w-6 h-6 cursor-pointer">
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            @error('question4') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                            <!-- Question 1.5 -->
                            <div class="flex justify-between items-center py-4">
                                <p class="text-gray-700 flex-1 pr-8">1.5 His/her skill in sustaining the interest of the learners</p>
                                <div class="flex gap-12">
                                    @foreach(['VS', 'S', 'F', 'P', 'NI'] as $value)
                                    <label class="flex items-center justify-center w-16">
                                        <input type="radio" wire:model="question5" value="{{ $value }}" class="w-6 h-6 cursor-pointer">
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            @error('question5') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div class="mt-8">
                            ...
                            <div class="flex justify-center gap-4">
                                {{-- Return: head → Experience, others → Interview page 2 --}}
                                <button type="button" wire:click="returnToPrevious"
                        ...
                        <div class="space-y-6">
                            ...
                            <!-- Question 2.1 -->
                            <div class="flex justify-between items-center py-4">
                                <p class="text-gray-700 flex-1 pr-8">2.1 The manifestation of composed behavior while teaching</p>
                                <div class="flex gap-12">
                                    @foreach(['VS', 'S', 'F', 'P', 'NI'] as $value)
                                    <label class="flex items-center justify-center w-16">
                                        <input type="radio" wire:model="personalQuestion1" value="{{ $value }}" class="w-6 h-6 cursor-pointer">
                                    </label>
                                    @endforeach
                                </div>
                            </div>
                            @error('personalQuestion1') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
        ...
        init() {
            window.addEventListener('show-swal-confirm', () => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) { @this.call('savePerformance'); }
                });
            });
